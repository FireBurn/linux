...
/// The tests themselves. `#[kunit_tests]` rewrites the module it is applied to, so it has
/// to see the items -- it rejects a file module, and an `include!` body leaves it nothing to
/// rewrite, which compiles green while registering no tests at all.
...
mod protocol {
    ...
    fn an_endpoint_list_logs_on_one_line() -> Result {
        ...
        let rendered = kernel::str::CString::try_from_fmt(kernel::prelude::fmt!(
            "{}",
            crate::HexList(&profile::PROFILE_NAVARRO.video_eps)
        ))?;
    ...
    fn diagonal_ctm_matches_the_general_matrix() -> Result {
        ...
        let fast = color::ColorPipeline::build(None, Some(&ctm_diag(CTM_ONE, CTM_HALF, CTM_ONE)))
            .ok_or(EINVAL)?;
    ...
    fn colour_frame_ep08_damage_selects_changed_strips() -> Result {
        ...
        let (d1, _) = video::wht::colour_frame_ep08_damage(geom, w, h, 0, 0, &[(1, 1, 2, 2)], g)?;
        assert!(!d1.is_empty());
        ...
        let (d2, _) = video::wht::colour_frame_ep08_damage(geom, w, h, 0, 0, &[(0, 0, 1, h)], g)?;
        assert!(total(&d1) < total(&d2) && total(&d2) < total(&full));

        // Non-aligned geometry is rejected (same contract as colour_frame_ep08).
        assert!(
            video::wht::colour_frame_ep08_damage(geom, 100, 32, 0, 0, &[(0, 0, 1, 1)], g).is_err()
        );
        Ok(())
    }
    ...
    fn stream_manage_restatement_matches_dlm() -> Result {
        ...
        for head in 0..2u8 {
            // Ridge: the connector is a one-based head number at offset 23, and the
            // content-stream id at offset 36 is 8 for head 0 and 9 for head 1.
            let c = cp::stream_manage_restatement(0, head, 8 + u16::from(head), false)?;
            assert_eq!(c.len(), 48);
    ...
    fn dual_nivo_rides_the_flags_word() -> Result {
        // Bit 2 of offset 42 declares that this head's video endpoint carries a second connector.
        // It must not disturb anything else in the word.
        let base = cp::Timing {
            hactive: 2560,
            hblank: 160,
            hsync_front: 48,
        ...
        let plain = cp::set_mode(0x30, 0, &base)?;
        let shared = cp::set_mode(
            0x30,
            0,
            &cp::Timing {
                dual_nivo: true,
    ...
    fn per_head_selectors_match_dlm_in_the_far_sockets() -> Result {
        // `head`, `head + 1` and `1 << head` agree for heads 0 and 1, so only a capture with a
        // monitor in a later socket separates them. These are the bytes DLM sends for heads 1
        // and 2.

        // `id=0x16 sub=0x23` names the head twice, at offset 22 and offset 23.
        for head in 0..drm_sink::HEADS as u8 {
            let req = cp::edid_engage_req(0x30, head)?;
            assert_eq!(req[22], head);
            assert_eq!(req[23], head);
        }

        // `id=0x15 sub=0x53` carries a head bitmask at offset 22: 2 for head 1 and 4 for head 2,
        // where a one-based index would send 3.
        assert_eq!(cp::post_edid_query(0x30, 1)?[22], 2);
        assert_eq!(cp::post_edid_query(0x30, 2)?[22], 4);
        for head in 0..drm_sink::HEADS as u8 {
            assert_eq!(cp::post_edid_query(0x30, head)?[22], 1u8 << head);
        }
        Ok(())
    }
    ...
    fn cursor_messages_structure() -> Result {
        ...
        // and two little-endian u16 fields at 24 and 26. Check the selector against more than one
        // head: `1 << head` and a fixed byte agree for a single head and diverge past it.
        ...
        // The selector is a bitmask, not a one-based index: the original two-entry table was
        // `[0x01, 0x02]`, which is `1 << head` for the only two heads that dock had.
        for head in 0..drm_sink::HEADS as u8 {
            let m = cp::cursor_move(1, head, 0, 0, true)?;
            assert_eq!(m[22], 1u8 << head);
        }
    ...
    fn the_first_real_mode_follows_the_prelude_rather_than_the_capture() {
        let last = drm_sink::NAVARRO_COLD_PRELUDE_END_MS;
        assert_eq!(last, 2016);
        assert!(drm_sink::NAVARRO_REAL_MODE_H0_MS > last);
    ...
    fn mode_ceilings_bound_bandwidth_not_refresh() {
        let refresh_ok = |p: &DockProfile, hz: i32| hz <= 0 || (hz as u32) <= p.max_refresh_hz;
    ...
    fn stride_and_vic_word_match_the_vendor_rules() -> Result {
        ...
        for (hactive, expect) in strides {
            assert_eq!(cp::render_stride(hactive), expect);
        }
        ...
        for (vic, expect) in words {
            assert_eq!(cp::vic_word(vic), expect);
        }
        Ok(())
    }
    ...
    fn sync_flags_follow_mode_polarity() -> Result {
        ...
        for (hact, htotal, hss, hse, vact, vtotal, vss, vse, clock, flags, off42) in cases {
            ...
            assert_eq!(cp::timing_from_drm_mode(&mode, false)?.sync_flags, off42);
            assert_eq!(cp::timing_from_drm_mode(&mode, true)?.sync_flags, off42);
        }
        // The teardown form carries none of it.
        let w = cp::clear_mode(3, 0)?;
