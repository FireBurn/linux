...
/// Automatic updates attempted per device: the device-name hash in the high 32 bits, the attempt
/// count in the low 32.
///
/// A dock that re-enumerates still reporting the old version is otherwise rewritten on every
/// probe, without end. The count has to survive the re-enumeration a write causes, so it cannot
/// live in driver data. One slot per dock that can be attached at once: a single slot lets two
/// docks evict each other, and neither ever reaches the limit that breaks the loop.
#[cfg(CONFIG_RUST_FW_LOADER_ABSTRACTIONS)]
const UPDATE_SLOTS: usize = 4;
#[cfg(CONFIG_RUST_FW_LOADER_ABSTRACTIONS)]
static UPDATE_ATTEMPTS: [AtomicU64; UPDATE_SLOTS] = [const { AtomicU64::new(0) }; UPDATE_SLOTS];
#[cfg(CONFIG_RUST_FW_LOADER_ABSTRACTIONS)]
...
/// The slot holding `key`'s count: its own if it has one, otherwise a free slot, otherwise one
/// shared by hash so that the count stays bounded even with more docks than slots.
#[cfg(CONFIG_RUST_FW_LOADER_ABSTRACTIONS)]
fn attempt_slot(key: u64) -> &'static AtomicU64 {
    let mut free = None;
    for slot in &UPDATE_ATTEMPTS {
        let v = slot.load(Ordering::Acquire);
        if v & ATTEMPT_KEY_MASK == key {
            return slot;
        }
        if v == 0 && free.is_none() {
            free = Some(slot);
    ...
    free.unwrap_or(&UPDATE_ATTEMPTS[(key >> 32) as usize % UPDATE_SLOTS])
}
...
fn update_attempts_left(dev: &Device) -> bool {
    ...
    let v = attempt_slot(key).load(Ordering::Acquire);
    v & ATTEMPT_KEY_MASK != key || (v as u32) < MAX_UPDATE_ATTEMPTS
}

#[cfg(CONFIG_RUST_FW_LOADER_ABSTRACTIONS)]
/// Count another automatic write against `dev`, or report that it has had its share.
///
/// Two interfaces of the same dock probe concurrently, so the count is claimed with a
/// compare-exchange: a plain read-modify-write lets both read the same value and both write one
/// back, and the limit never counts past one.
fn claim_update_attempt(dev: &Device) -> bool {
    let key = attempt_key(dev);
    let slot = attempt_slot(key);
    loop {
        let cur = slot.load(Ordering::Acquire);
        let count = if cur & ATTEMPT_KEY_MASK == key {
            cur as u32
        } else {
            0
        };
...
fn clear_update_attempts(dev: &Device) {
    ...
    let slot = attempt_slot(key);
    let cur = slot.load(Ordering::Acquire);
    if cur & ATTEMPT_KEY_MASK == key {
        let _ = slot.compare_exchange(cur, 0, Ordering::AcqRel, Ordering::Acquire);
    }
}
...
pub(crate) fn update_if_newer(
    ...
    if !is_package(fw.data()) {
        dev_warn!(
            dev,
            "{} is not a DisplayLink firmware package; leaving the dock on {}\n",
            name,
            identity.version
        );
        return Ok(());
    }
    // The package says which dock it is for. Writing another family's image is the one mistake
    // here that cannot be undone, so it is refused before the dock is touched -- the same check
    // the userspace upload path makes.
    match package_family(fw.data()) {
        Some(f) if Some(f) == identity.family() => {}
        _ => {
            dev_warn!(
                dev,
                "{} is not for this dock family; leaving the dock on {}\n",
                name,
                identity.version
            );
            return Ok(());
