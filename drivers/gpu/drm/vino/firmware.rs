...
///
/// ⚠ Only `NavaDock` has been read off real hardware; the identity spellings for the other three
/// come from the vendor's own platform names and are unverified.
#[derive(Clone, Copy, PartialEq, Eq)]
...
pub(crate) fn update_if_newer(
    ...
    if packaged <= identity.version && !force {
        dev_info!(
            dev,
            "vino: firmware is current ({} offers {})\n",
            name,
            packaged
        );
        return Ok(());
