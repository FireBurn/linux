...
unsafe fn transform_inblock_avx2(block: &[i32; PIXELS]) -> [i32; COEFFS] {
    ...
    unsafe {
        ...
        for i in 0..4 {
            ...
            _mm_storeu_si128(
                ll1.as_mut_ptr().add(i * 4) as *mut __m128i,
                _mm_add_epi32(la, lb),
            );
            _mm_storeu_si128(
                lh1.as_mut_ptr().add(i * 4) as *mut __m128i,
                _mm_sub_epi32(la, lb),
            );
            _mm_storeu_si128(
                hl1.as_mut_ptr().add(i * 4) as *mut __m128i,
                _mm_add_epi32(ha, hb),
            );
            _mm_storeu_si128(
                hh1.as_mut_ptr().add(i * 4) as *mut __m128i,
                _mm_sub_epi32(ha, hb),
            );
        }
...
pub(crate) fn bench() -> Result {
    ...
    pr_info!(
        "vino-simd: scalar {scalar_ns} ns/block over {} blocks\n",
        REPS * BLOCKS
    );
    ...
    if has_avx2() {
        ...
        if bad != 0 {
            pr_err!(
                "vino-simd: avx2-inblock MISMATCH on {} blocks -- not reporting a speedup\n",
                bad
            );
            return Err(EINVAL);
        }
    ...
    run!("avx2", has_avx2(), transform_avx2, __m256i, 8, {
        // SAFETY: an all-zero bit pattern is a valid `__m256i`.
        unsafe { core::mem::zeroed() }
    });
    run!("avx512", has_avx512(), transform_avx512, __m512i, 16, {
        // SAFETY: an all-zero bit pattern is a valid `__m512i`.
        unsafe { core::mem::zeroed() }
    });
