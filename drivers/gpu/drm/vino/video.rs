...
pub(crate) mod wht {
    ...
    /// 64 transform coeffs. **BYTE-EXACT vs DLM** on the controlled signature library.
    ...
    pub(crate) fn ac_strip(coeff: &[i32; COEFFS], x: u16, y: u16) -> Result<KVec<u8>> {
        ...
        // SYNC and DC are one continuous LSB-first bitstream.  There is no byte
        // boundary between them: on arbitrary content DLM's first DC token can
        // begin in the middle of the final SYNC byte.  Keeping separate `Bits`
        // accumulators inserted up to seven zero bits and shifted every AC row.
        let mut main = Bits::new();
        for _ in 0..16 {
            main.sync_unit(last)?;
        }
        main.esc(0, SOLID_DC_CMAX)?; // Cr=0
        main.esc(0, SOLID_DC_CMAX)?; // Cb=0
        main.esc(q[0], SOLID_DC_CMAX)?; // Y absolute (block 0)
        for _ in 0..45 {
            main.bit(0)?; // 15 DPCM-zero blocks x (Cr,Cb,Y)
        }
    ...
    /// every coding layer. This historical entry point accepts luma coefficients only; callers
    /// requiring colour use [`colour_strip`], which carries the complete Cr/Cb/Y grammar.
    pub(crate) fn strip(
        blocks: &[[i32; COEFFS]; STRIP_BLOCKS],
        x: u16,
        y: u16,
    ) -> Result<KVec<u8>> {
        ...
        let mut main = Bits::new();
        for k in 0..STRIP_BLOCKS {
            main.sync_unit(lasts[k])?;
        }
        dc_plane(&mut main, &dc)?;
    ...
    /// at `[oy*stride + ox]`) into its 64 Haar coefficients.
    ...
    /// This achromatic helper is retained for the historical luma corpus; live scanout uses
    /// [`colour_frame_ep08`].
