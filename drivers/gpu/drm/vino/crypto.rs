...
/// An AES-128 key expanded once, for the sealed control plane's AES-CTR.
pub(super) use kernel::crypto::Aes128;

/// `AES_ECB(key, block)` -- one 16-byte AES-128 block, which HDCP 2.2 needs for the
/// dKey derivation. That is the only bare block cipher left here: the control plane's
/// AES-CTR runs on a key held expanded for the whole session.
