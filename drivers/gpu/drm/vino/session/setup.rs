    ...
    pub(crate) fn send_cp_setup(
        ...
        match out_q.as_mut() {
            Some(q) => {
                ...
                if profile.protocol.reply_discipline == profile::ReplyDiscipline::Lockstep {
                    // DLM advances as soon as the dock authenticates msg0, so take the reply
                    // directly. Draining empty 10-ms read windows first moves every following
                    // setup transition roughly 90 ms later on the wire.
        ...
        if profile.protocol.per_connector_onehot {
            ...
            for connector in 0..connector_count {
                ...
                let stream_id = profile.geometry().stream_id(connector as u8);
                let vkey = cp::VideoKey::new(&video_keys[connector])?;
                let content = cp::stream_open(profile.protocol.stream_marker_kind);
                let open = cp::seal_video_arm(&vkey, stream_id, 0x000a, 0, &content)?;
