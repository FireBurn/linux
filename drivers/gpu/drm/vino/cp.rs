...
/// A control-plane session key, with its AES schedule expanded once.
///
/// Expanding an AES key costs far more than encrypting a block, and every sealed
/// message on a session uses the same key, so the schedule is held with the
/// session rather than rebuilt per message. The raw bytes are kept alongside it
/// because the Dl3Cmac prepares a key of its own from them.
pub(super) struct SessionKey {
    raw: kernel::crypto::Secret<16>,
    aes: crypto::Aes128,
}

impl SessionKey {
    /// Expands `key`, the whitened session key from [`cp_session_key`].
    pub(super) fn new(key: [u8; 16]) -> Result<Self> {
        Ok(Self {
            aes: crypto::Aes128::new(&key)?,
            raw: kernel::crypto::Secret::new(key),
        })
    }

    /// The raw key bytes, for the Dl3Cmac and for handing the session on.
    pub(super) fn raw(&self) -> &[u8; 16] {
        &self.raw
    }

    /// Encrypts or decrypts `data` in place with the frame's counter block.
    fn ctr(&self, riv: &[u8; 8], seq: u32, data: &mut [u8]) {
        self.aes.ctr(&mut ctr_block(riv, seq), data);
    }
}

/// A connector's video sealing key: the whitened `SKE_Send_Eks` key with its schedule
/// expanded, and the content nonce its stream counts from.
///
/// Every sealed record on the connector uses both, so they are split out of the stored
/// blob once and held together thereafter.
...
impl VideoKey {
    /// Expands the key at the front of a stored blob and keeps the nonce behind it. The blob
    /// is laid out as key(16) || nonce(8) || pad(8); the pad is unused.
    pub(super) fn new(blob: &kernel::crypto::Secret<32>) -> Result<Self> {
        let mut key = [0u8; 16];
        key.copy_from_slice(&blob[..16]);
        let mut nonce = [0u8; 8];
        nonce.copy_from_slice(&blob[16..24]);
        Ok(Self {
...
pub(super) fn inner_plaintext(ks: &SessionKey, out_riv: &[u8; 8], wire: &[u8]) -> Option<KVec<u8>> {
    if wire.len() <= 16 {
        return None;
    }
...
pub(super) fn decode_in_lenient(
    ...
    for riv in inbound_reply_rivs(out_riv) {
        ...
        let sub = u16::from_le_bytes([plaintext[2], plaintext[3]]);
        let ctr = u16::from_le_bytes([plaintext[4], plaintext[5]]);
        // Navarro's device-log/status replies use session-varying IDs beyond the catalogued
...
/// The initial AES-CTR counter block: the delivered riv, four zero bytes, and the frame's
/// sequence number big-endian in the low four bytes.
///
/// The dock counts blocks in that 32-bit field. `Aes128::ctr` advances the whole 128-bit block,
/// which is the same thing until the field would carry into the zero bytes above it -- 2^32
/// blocks, or 64 GiB, into a single control session.
fn ctr_block(riv: &[u8; 8], seq: u32) -> [u8; 16] {
    let mut ctr = [0u8; 16];
    ctr[..8].copy_from_slice(riv);
    ctr[12..].copy_from_slice(&seq.to_be_bytes());
    ctr
}
...
pub(super) fn seal_livemac(
    ...
    ct.extend_from_slice(content_pt, GFP_KERNEL)?;
    ks.ctr(riv, seq, &mut ct);
    let tag = dl3cmac_tag(ks.raw(), riv, seq as u64, &ct)?;
...
pub(super) const CP_SETUP_PER_HEAD: [(u16, u16, usize); 9] = [
    ...
    (0x0019, 0x0031, 32), // per-connector strm2 -- connector @off22, 06 [connector*4] 04 @off24
];
...
pub(super) fn seal_video_arm(
    ...
    seal_livemac(&vkey.key, &vkey.nonce, &hdr, content)
}
...
pub(super) fn open_in(
    ...
    let expected = dl3cmac_tag(ks.raw(), in_riv, seq as u64, ct)?;
    ...
    plaintext.extend_from_slice(ct, GFP_KERNEL)?;
    ks.ctr(in_riv, seq, &mut plaintext);
    Ok(plaintext)
}
...
mod tests {
    ...
    fn seal_livemac_roundtrip() -> Result {
        ...
        assert_eq!(&frame[16 + 32..], &dl3cmac_tag(ks.raw(), &riv, 4, ct)?[..]);
    ...
    fn reply_decoders_accept_all_supported_rivs() -> Result {
        let ks = SessionKey::new([0x5au8; 16])?;
    ...
    fn video_arm_burst_table_framing() -> Result {
        ...
        let mut blob = kernel::crypto::Secret::new([0x5au8; 32]);
        blob[16..24].fill(0x11);
        let vkey = VideoKey::new(&blob)?;
        for (i, &(wire_type, sub_base, aux, body_len)) in VIDEO_ARM_BURST.iter().enumerate() {
            ...
            if wire_type == 2 {
                ...
                let frame = seal_video_arm(&vkey, sub_base, aux, 0, &content)?;
    ...
    fn navarro_stream_open_matches_the_wire() -> Result {
        ...
        let vkey = VideoKey::new(&kernel::crypto::Secret::zeroed())?;
        let frame = seal_video_arm(&vkey, 0x0007, 0x0002, 0, &open)?;
