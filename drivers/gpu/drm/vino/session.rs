...
impl VinoDriver {
    ...
    pub(super) fn bring_up(dev: &UsbLink<'_>, profile: &DockProfile) -> Result {
        ...
        match dev.control_recv(
            ...
            // "RidgeDoc", matching ridge-dock-release.spkg, and a third-party USB3 HDMI/DVI
            // adapter reads "EllaDock", matching ella-dock-release.spkg.
            Ok(()) => {
                let mut ascii = [b'.'; 16];
        ...
        macro_rules! send_init {
            ($id:expr, $sub:expr, $fixed_prefix:expr) => {{
                ...
                let d = if profile.is_navarro() {
                    Self::lockstep_reply(dev, ep84_q.as_mut(), &mut resp, session, cp_ctr, edid_out)
                } else {
                    Self::drain_ep84(
                        dev,
                        ep84_q.as_mut(),
