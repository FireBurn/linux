...
/// ⚠ **Every head field below is a transcript SLOT**: 0 is the first head an activation brings up
/// and 1 the second, whichever sockets they occupy. Both timelines were recorded with the panels
/// in the first two sockets, where a slot and a head number happen to coincide.
...
///
/// DLM's capture put it at 2978 ms, but DLM's own prelude finishes at 2016 ms. The ~960 ms
/// between is dead air in that capture, not something the dock asked for, and it was 53% of a
/// 5.59 s enumerate-to-pixels bring-up. Follow the end of the prelude plus one poll interval, so
/// the two cannot drift apart when the prelude changes.
...
impl VinoDrmData {
    ...
    fn blank_head(&self, dev: &BoundInterface<'_>, head: u8) -> Result {
        ...
        //
        // ⛔ It is **not** Ridge's close bracket (`2f=0`, `2e=0`), which re-enumerates this dock
        // about two seconds later, measured seven times out of seven.
        //
    ...
    ///
    /// ⚠ **Every head number in a [`ColdTimeline`] is a transcript slot, not a head**: slot 0 is
    /// the lowest-numbered activating head and slot 1 the next. They are resolved to real heads at
    ...
    fn activate_dual_wake(
        ...
        let timeline = (|| -> Result<(u32, u32)> {
            ...
            for slot in 0..slots.len() {
                ...
                // ⭐ Recompute the shared-endpoint declaration at send time, not at
                // `atomic_enable` time. Whether this head's video endpoint carries a second stream
                // is a property of the *other* head, and it changes while this head's timing sits
                // cached: a monitor plugged into the partner socket a moment later left the first
                // head's mode set saying "not shared" forever, and only one of the two panels lit.
    ...
    pub(super) fn set_disconnected(&self, head: usize) {
        ...
        if self.heads_present.load(Ordering::Acquire) & (1u32 << head) != 0 {
            pr_info!(
                "vino: [{}conn] socket {socket} presence CLEARED (connector goes disconnected)\n",
    ...
    pub(super) fn repair_flapped_head(&self, dev: &VinoDrmDevice, flapped: usize) -> u32 {
        ...
        pr_info!("vino: head {flapped} sink flap -- re-driving {queued} lit head(s) together\n");
        queued
    }
...
///
/// The deadline used to be checked only when a caller arrived to start a transaction, which is
/// too late by construction: the thread that would arrive is the keepalive, and the keepalive is
/// the thread that gets stuck. A measured wedge reported 15787 ms against a 5000 ms limit, and
/// only because the dock re-enumerated. This runs on the system queue, so it is scheduled whatever
/// vino's own queues are doing, and it touches nothing a stuck transfer can be holding.
