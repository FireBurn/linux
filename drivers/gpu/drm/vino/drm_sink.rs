...
pub(super) struct VinoDrmData {
    ...
    /// Per-connector key and nonce used to seal pipe-arm records, expanded once.
    #[pin]
    video_keys: Mutex<[Option<Arc<crate::cp::VideoKey>>; MAX_CONNECTORS]>,
...
impl VinoDrmData {
    ...
    pub(super) fn new(
        ...
        try_pin_init!(Self {
            ...
            video_keys <- new_mutex!(core::array::from_fn(|_| None)),
    ...
    pub(super) fn set_video_keys(
        ...
        let mut expanded: [Option<Arc<crate::cp::VideoKey>>; MAX_CONNECTORS] =
            core::array::from_fn(|_| None);
        for (slot, blob) in expanded.iter_mut().zip(keys.iter()) {
            *slot = Some(Arc::new(crate::cp::VideoKey::new(blob)?, GFP_KERNEL)?);
        }
        *self.video_keys.lock() = expanded;
