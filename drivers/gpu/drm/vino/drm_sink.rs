...
impl VinoDrmData {
    ...
    /// clear it (identity) with `None`. Each channel is reduced to 8 bits.
    pub(super) fn update_gamma(&self, head: usize, lut: Option<&[crtc::ColorLut]>) {
        ...
        if changed {
            if cached.is_some() {
                vino_debug!("vino: head {head} gamma LUT updated\n");
            } else {
                vino_debug!("vino: head {head} gamma LUT cleared\n");
            }
...
fn direct_pixel_map(
    ...
    gamma.is_none() && rotation == plane::Rotation::ROTATE_0 && output_w == w && output_h == h
}
