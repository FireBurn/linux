...
// Re-exported for the KUnit tests, which drive the rotation mapping and the parallel-encode
// equivalence check directly rather than through a live commit.
#[cfg(CONFIG_DRM_VINO_KUNIT_TEST)]
pub(super) use mode_objects::rot_src;
#[cfg(CONFIG_DRM_VINO_KUNIT_TEST)]
pub(super) use scanout::parallel_rotation_matches_serial;
...
///
/// Issued per presentation this would be 200-600 CP round-trips a second across two heads at
/// ~100 fps, every one of them serialising on the single control link, and whichever head looped
/// fastest would re-acquire it immediately and starve the other. DLM issues ~3.8 per second, so a
/// quarter-second floor matches the reference and leaves the link free for the other head.
...
/// Every head field below is a transcript slot, not a head number: 0 is the first head an
...
enum NavarroColdOp {
    ...
    Probe(u8),
    Fetch(u8),
    /// Tear the downstream sink down. Offset 23 is the literal state `0xff`, not a head.
    SinkTeardown(u8),
    /// Engage the downstream sink.
    ///
    /// `id=0x16 sub=0x23` names the head twice, at offset 22 and at offset 23, which is why
    /// it is [`super::cp::edid_sink_state`]`(head, head)`. It is a distinct variant rather than a
    /// state so that the remap below cannot mistake the second selector for a constant: the dock
    /// acknowledges a mismatched pair and then never enables the sink.
    Engage(u8),
    PostEdid(u8),
    Clear(u8),
}
...
const NAVARRO_REAL_MODE_SETTLE_MS: i64 = 20;

/// When the cold prelude's last operation is due.
pub(super) const NAVARRO_COLD_PRELUDE_END_MS: i64 =
...
pub(super) const NAVARRO_REAL_MODE_H0_MS: i64 =
...
///
/// Measured in silence rather than in unanswered messages, because those are not the same thing.
/// A head whose sink is down leaves its own re-engage unanswered every few seconds for the life of
/// the session while its sibling drives a lit panel and the status dialogue replies throughout;
/// counting messages tears that session down. A lit, idle link answers continuously, so real
...
pub(super) struct VinoDrmData {
    ...
    /// Windows holding one head at 8 bits while the other ran at 10.
...
impl VinoDrmData {
    ...
    fn abandon_cp_session(&self, silent_ms: i64) {
        if self
            .cp_session_live
    ...
    fn check_cp_session(&self) -> Result {
        if !self.cp_link_alive() {
            return Err(ENODEV);
        }
        let silent_ms = self.cp_silent_for_ms();
        if silent_ms >= CP_SILENCE_LIMIT_MS {
    ...
    pub(super) fn drop_connectors_with_session(&self, drm_dev: &VinoDrmDevice) {
        ...
        for head in 0..self.connector_count() {
            if !self.runtime_connector(head) || !self.head_present(head) {
                continue;
            }
            self.set_disconnected(head);
            dropped = true;
            pr_warn!(
    ...
    /// only be established through probe. A reset re-enumerates and re-probes, which is what
    /// cycling `authorized` by hand does, and it is the one recovery a stuck transfer cannot
    /// block: the USB core runs it from its own work item.
    fn reset_after_wedge(&self) {
        if self
            .cp_reset_queued
    ...
    pub(super) fn start_cp_watchdog(&self, drm_dev: &VinoDrmDevice) {
    ...
    fn navarro_cold_op(&self, dev: &BoundInterface<'_>, op: NavarroColdOp) -> Result {
        match op {
            ...
            NavarroColdOp::SinkTeardown(head) => self.send_cp(dev, 0x16, 0, |ctr| {
                super::cp::edid_sink_state(ctr, head, 0xff)
            }),
            NavarroColdOp::Engage(head) => {
                self.send_cp(dev, 0x16, 0, |ctr| super::cp::edid_engage_req(ctr, head))
            }
    ...
    fn blank_head(&self, dev: &BoundInterface<'_>, head: u8) -> Result {
        ...
        // and off. An idle screen blank alone therefore puts the dock into a reset loop and takes
        // the whole desktop down with it, which is far worse than a panel that keeps showing its
        // last frame while the compositor thinks it is off.
        //
        ...
        // Navarro's disable is two markers and then silence: no video, no mode set and no close
        // bracket for as long as the output stays down. It is the same pair `modeset_bracket_pre`
        // opens with, so the stream is held rather than torn down.
        //
        // It is not Ridge's close bracket (`2f=0`, `2e=0`), which re-enumerates this dock about
        // two seconds later.
        //
        // [`Self::close_blank_bracket`] owes the matching re-open before the head is driven again.
        if self.is_navarro() {
            ...
            self.blank_bracket_open
                .fetch_or(1u32 << head_i, Ordering::Release);
            vino_debug!(
                "vino: socket {socket} blanked (Navarro: bracket held open, stream idle)\n"
            );
            return Ok(());
        }
        ...
        if self.is_navarro() {
            vino_debug!(
                "vino: socket {socket} blanked to black ({sent} presentation(s)); stream left open                  -- Navarro's close sequence is not established\n"
            );
            return Ok(());
        }
    ...
    /// Close a blank bracket before this head is driven again.
    ///
    /// The sequence is `2f=1`, `2e=0`, then `2f=0`, `2e=0`, and it precedes the EDID probe and the
    /// mode set that follow. A head that was never blanked costs nothing here.
    fn close_blank_bracket(&self, dev: &BoundInterface<'_>, head: u8) -> Result {
        let socket = head + 1;
        let bit = 1u32 << head;
        if self.blank_bracket_open.fetch_and(!bit, Ordering::AcqRel) & bit == 0 {
            return Ok(());
        }
        self.stream_marker(dev, head, 0x2f, 1)?;
        self.stream_marker(dev, head, 0x2e, 0)?;
        self.stream_marker(dev, head, 0x2f, 0)?;
        self.stream_marker(dev, head, 0x2e, 0)?;
        // A wake is a repair, not a cold plug: the downstream link is still trained and the blank
        // only held the bracket open, so the three-second keyframe window `sustain_window` grants a
        // cold activation buys nothing and costs about a gigabyte per head. Sustained bandwidth is
        // also what destabilises this dock, so it is never spent twice.
        self.repair_heads.fetch_or(bit, Ordering::Release);
    ...
    fn submit_prompt_training(
        ...
        vino_debug!(
            "vino: head={head} prompt-training parameter map {} B\n",
            params.len()
        );
        let arm = if with_arm {
    ...
    fn activate_head(
        ...
        // Same as the dual path: a head coming back from a blank has a bracket owed before
        // anything else is sent to it.
        self.close_blank_bracket(dev, head)?;
        // The caller's timing was built when this head was enabled, possibly before its endpoint
        // partner existed; see `activate_dual_wake`.
        let timing = &super::cp::Timing {
            dual_nivo: self.endpoint_is_shared(head_i),
            ..*timing
        };
    ...
    ///
    /// Every head number in a [`ColdTimeline`] is a transcript slot, not a head: slot 0 is the
    /// lowest-numbered activating head and slot 1 the next, and they are resolved to real heads at
    /// the point of send. Taken literally they address heads 0 and 1, whose bits are absent from
    /// `sent` for any other pair of sockets, so no marker and no video would go out at all.
    fn activate_dual_wake(
        ...
        let mut ordinary_prompts: [Option<KVec<KVec<u8>>>; HEADS] = core::array::from_fn(|_| None);
        ...
        // The two heads this activation is about, in the order the timeline brings them up. Both
        // cold timelines describe exactly two heads, so a third is reported rather than silently
        // left out of the choreography.
        let mut slots = [0u8; 2];
        let mut n = 0;
        for head in 0..HEADS {
        ...
        if n > slots.len() {
            pr_warn!(
                "vino: {n} heads activating but the cold timeline describes {}; choreographing {} and {}\n",
                slots.len(),
                slots[0],
                slots[1]
            );
        }
        // Slot -> head. Out-of-range slots cannot occur (both timelines only name 0 and 1) but
        // clamp rather than panic, because a timeline is data and this runs under the CP lock.
        let head_of = |slot: u8| usize::from(slots[usize::from(slot).min(slots.len() - 1)]);
        ...
        for head in 0..HEADS {
            if valid & (1u32 << head) != 0 {
                self.close_blank_bracket(dev, head as u8)?;
            }
        }
        self.begin_cp_timeline();
        ...
        let timeline = (|| -> Result<(u32, u32)> {
            if self.is_navarro() {
                let remap = |slot: u8| head_of(slot) as u8;
            ...
            let navarro_counters = if self.is_navarro() {
            ...
            for slot in 0..slots.len() {
                ...
                let Some(mut timing) = timings[head] else {
                    continue;
                };
                // Recompute the shared-endpoint declaration at send time, not at `atomic_enable`
                // time. Whether this head's video endpoint carries a second stream is a property
                // of the *other* head, and it can change while this head's timing sits cached: a
                // monitor plugged into the partner socket a moment later would otherwise leave
                // this mode set saying "not shared", and only one of the two panels lights.
                timing.dual_nivo = self.endpoint_is_shared(head);
                // The second head's mode set is spaced from the first by this dock's measured
                ...
                if let Some(counters) = navarro_counters.as_ref() {
                    ...
                    // the head number these collided for any pair but (0, 1), and Navarro NAKs
            ...
            // Vino does not: this dock re-enumerates when a stream is driven at an empty head, and
            // an empty socket's index is not stable across bring-ups.

            for &(vslot, at) in timeline.video {
                let head = head_of(vslot as u8);
                cp_until!(at - 1);
                // Some docks set a head's mode a second time shortly before its video.
                for &(off, reslot) in timeline.remode {
                    let rehead = head_of(reslot as u8);
                    if off >= at || remoded & (1u32 << rehead) != 0 || sent & (1u32 << rehead) == 0
                    {
                        continue;
                    }
                    let Some(mut timing) = timings[rehead] else {
                        continue;
                    };
                    timing.dual_nivo = self.endpoint_is_shared(rehead);
                    cp_until!(off - 1);
                    Self::wait_mode_offset(anchor, off);
                    self.send_cp(dev, 0x48, 0, |ctr| {
    ...
    fn build_frame_opener(&self, head: u8, seq0: u32, prologue: bool) -> Option<[u8; 32]> {
        if self.uses_arm_burst() || prologue {
            ...
            Some(super::video::wht::navarro_frame_opener(
                self.geometry(),
                head,
                seq0,
            ))
    ...
    fn build_stream_report_buf(&self, head: usize) -> Result<Option<KVec<u8>>> {
        ...
        let (aux, content): (u16, KVec<u8>) = if with_mode {
            let timing = self
                ...
                .ok_or(ENODEV)?;
            let header =
                super::video_arm::mode_header(timing.hactive, timing.vactive, NAVARRO_LAYOUT_WORD);
            let mut v = KVec::new();
    ...
    fn send_cp_reply<T>(
        ...
        if let Err(e) = dev.ctrl_send(&frame, super::timeout(), GFP_KERNEL) {
            let silent_ms = self.cp_silent_for_ms();
            if silent_ms >= CP_SILENCE_LIMIT_MS {
                self.abandon_cp_session(silent_ms);
                *guard = None;
    ...
    /// cannot read it.
    ///
    /// A DP-to-HDMI converter that mangles or drops DDC leaves the dock unable to read the monitor at
    ...
    /// fbdev emulation a blank cheque, and its 640x480 default mode set puts this dock into a 25 s
    /// re-enumeration loop. Userspace supplies the description and then forces the connector on
    /// (`echo on > .../status`), in that order, which is also what makes the sequence race-free.
    ///
    ...
    fn probe_connector_present(&self, dev: &BoundInterface<'_>, sel: u8, head: u8) -> Option<bool> {
        ...
        if dev.ctrl_send(&frame, super::timeout(), GFP_KERNEL).is_err() {
            let silent_ms = self.cp_silent_for_ms();
            if silent_ms >= CP_SILENCE_LIMIT_MS {
                self.abandon_cp_session(silent_ms);
                *guard = None;
        ...
        let got = loop {
            ...
            if n > 16 {
                match super::cp::decode_in_lenient(&link.ks, &link.riv, &reply[..n]) {
    ...
    pub(super) fn repair_flapped_head(&self, dev: &VinoDrmDevice, flapped: usize) -> u32 {
        ...
        for head in 0..HEADS {
            ...
            self.repair_heads.fetch_or(1u32 << head, Ordering::Release);
            self.modeset_active[head].store(0, Ordering::Release);
            self.owe_keyframe(head);
            self.strip_hashes.lock()[head] = None;
            self.dirty_ttl.lock()[head] = None;
        ...
        pr_warn!("vino: head {flapped} sink flap -- re-driving {queued} lit head(s) together\n");
        queued
    }
    ...
    /// Whether another connector on `head`'s video endpoint is also being driven.
    ///
    /// `0x08` owns connectors {0, 2} and `0x0a` owns {1, 3}, so a head's partner is the one two
    /// away. Such a pair must declare `Dual NIVO` in both mode sets or the dock drives only one of
    /// the two streams it is sent, however correctly they are tagged.
    pub(super) fn endpoint_is_shared(&self, head: usize) -> bool {
        if head >= HEADS || self.connector_count() <= 2 {
            return false;
        }
        let partner = head ^ 2;
        partner < HEADS && self.modeset_requested[partner].load(Ordering::Acquire) != 0
    }
...
fn run_cp_watchdog(this: ARef<VinoDrmDevice>) {
...
impl WorkItem for VinoDrmData {
    ...
    fn run(this: ARef<VinoDrmDevice>) {
        ...
        loop {
            ...
            // On the DL7400 a mode set is dock-wide, not per head. Configuring one connector while
            // any other is lit makes the dock re-enumerate about 100 ms after the next video
            // write, on every shape of the change: 120 -> 165, 165 -> 120 and 120 -> 60 on a live
            // head, waking a second head a second after the first, and reconfiguring a head whose
            // sibling has been lit and idle for minutes. The same changes with the sibling
            // disabled are clean, and so is the simultaneous `activate_dual_wake` path. DLM
            ...
            // This replays the cold choreography -- a dock-wide sink reset and pipe clears -- on a
            // dock that is already driving its sinks. That is the cost of the only mode set this
            // dock will take.
            if cmd_heads != 0 && data.is_navarro() {
                ...
                for head in 0..HEADS {
                    ...
                    if active != 0 && data.modeset_requested[head].load(Ordering::Acquire) == active
                    {
...
impl KmsDriver for VinoDrmDriver {
    ...
    fn create_objects(dev: &UnregisteredKmsDevice<'_, Self>) -> Result {
        ...
        for head in 0..HEADS {
            ...
            // If the hardware cursor ever has to go back to software, withdraw this plane rather
            // than merely stop sending the messages. A cursor plane whose atomic commit
