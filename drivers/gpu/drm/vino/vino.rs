...
struct Session {
    /// The control-plane key, expanded once: every sealed message on the session uses it.
    ks: cp::SessionKey,
    ...
    rsa: kernel::crypto::rsa::RsaPublicKey,
    rxid_list: KVec<u8>,
}
        ...
        for attempt in 1..=SESSION_ATTEMPTS {
            ...
            let result = (|| -> Result {
                ...
                data.publish_session(
                    dev,
                    cp::SessionKey::new(*session.ks.raw())?,
                    &session.riv,
                    wseq_end,
                    ctr_end,
...
impl usb::Driver for VinoDriver {
    ...
    fn probe<'bound>(
        ...
        let function = *info.ok_or(ENODEV)?;
