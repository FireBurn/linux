...
fn log_device_identity(
    ...
    for ep in intf.cur_altsetting().endpoints() {
        ...
        dev_info!(
            cdev,
            "vino:   ep {addr:#04x} {kind}-{dir} maxp {}\n",
            ep.maxp()
        );
    }
}
            ...
            let result = (|| -> Result {
                    ...
                    .take(usize::from(profile.connectors))
                {
                    ...
                    if let Some(blob) = slot {
                        ...
                        vino_dev_debug!(
                            cdev,
                            "vino: cached socket {socket} EDID ({n} bytes)\n",
                            socket = head + 1
                        );
                    }
                    ...
                    if have_edid {
                        ...
                        dev_info!(
                            cdev,
                            "vino: socket {socket} monitor connected\n",
                            socket = head + 1
                        );
                    }
                }
            ...
            while !data.is_shutting_down() {
                ...
                // unanswered CP messages every `REENGAGE_RETRY` for the life of the session.
                {
                    ...
                    for h in 0..data.connector_count() {
                        ...
                        vino_dev_debug!(
                            cdev,
                            "vino: socket {socket} absent -- retrying the sink re-engage\n"
                        );
                        ...
                        match data.reengage_head(dev, h as u8) {
                            ...
                            Err(e) => {
                                vino_dev_debug!(
                                    cdev,
                                    "vino: socket {socket} sink re-engagement failed ({e:?})\n"
                                )
                            }
                        }
                    ...
                    for h in 0..data.connector_count() {
                        ...
                        if present == head_known[h] {
                            ...
                            if present && head_absent_since[h].take().is_some() {
                                vino_dev_debug!(
                                    cdev,
                                    "vino: socket {socket} sink flap healed on its own\n"
                                );
                            }
                            continue;
                        }
                        ...
                        if present {
                            ...
                            next_reengage[h] = Instant::<Monotonic>::now() + REENGAGE_RETRY;
                            presence_grace[h] = Instant::<Monotonic>::now() + PRESENCE_GRACE;
                            dev_info!(cdev, "vino: socket {socket} monitor connected\n");
    ...
    fn probe<'bound>(
        ...
        if function == Function::Dfu {
            ...
            match (identity.as_ref().ok_or(ENODEV)).and_then(|id| {
                let link = io.enter()?;
                dev_info!(cdev, "vino: {id} running firmware {}\n", id.version);
        ...
        if function == Function::Dfu {
            dev_info!(
                cdev,
                "vino: bound interface {ifnum} (idle -- control is the display function)\n"
            );
        ...
        let profile = match identity_family {
            Some(family) => match profile::for_family(family) {
                ...
                None => {
                    ...
                    dev_info!(
                        cdev,
                        "vino: {id} is not a family this driver drives yet; declining. \
            ...
            None => {
                ...
                match profile::for_product(usbdev.product_id()) {
                    Some(profile) => {
                        dev_info!(
                            cdev,
                            "vino: identity descriptor unreadable; using the quirk entry for \
                             {:04x}\n",
                            usbdev.product_id()
                        );
                        profile
                    ...
                    None => {
                        dev_info!(
                            cdev,
                            "vino: no identity descriptor and no quirk entry; declining. \
                             A report makes it supportable: Documentation/gpu/vino.rst\n"
                        );
                        return Err(ENODEV);
                    }
        ...
        // Which profile the dock resolved to, and the endpoints it implies. On unfamiliar
        // hardware this one line is what says whether the driver recognised the dock, and needing
        // a debug build to see it costs a whole test round trip.
        dev_info!(
            cdev,
        ...
        if connectors != profile.connectors {
            dev_info!(
                cdev,
                "vino: {connectors} connector(s) backed by video endpoints, not the {} this \
