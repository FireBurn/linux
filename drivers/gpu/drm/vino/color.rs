...
//! per-pixel cost matters more than generality. A CTM that only scales each channel, which is
//! the shape every colour-temperature corrector produces, collapses into per-channel lookup
//! tables exactly as a gamma ramp does; [`ColorPipeline::Fused`] keeps that case at one table
//! lookup per channel, the same cost as before CTM existed. Only a matrix that genuinely mixes
//! channels pays for arithmetic.
