...
impl VinoDrmData {
    ...
    ///
    /// A presence flap is the dock really taking a sink down, and the repair stays below DRM:
    /// the connector remains registered. Dropping it so the compositor re-enables the output
    /// re-lays-out userspace, and the mode set that follows names one connector while its
    /// sibling is lit, which re-enumerates the dock.
    ///
