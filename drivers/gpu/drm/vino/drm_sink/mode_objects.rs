    ...
    fn atomic_enable(commit: CrtcAtomicCommit<'_, Self>) {
        ...
        let timing = match crate::cp::timing_from_drm_mode(new.mode(), data.is_navarro()) {
            Ok(mut timing) => {
                ...
                // Declare the shared video endpoint. Four connectors are multiplexed onto two
                // bulk endpoints, and DLM names bit 2 of the flags word `Dual NIVO`; a head whose
                // partner connector is also live has to say so, or the dock drives only one of
                // the two streams it is being sent.
                timing.dual_nivo = data.endpoint_is_shared(head as usize);
                timing
            }
            Err(e) => {
                pr_err!(
