...
impl VinoDrmData {
    ...
    /// This connector's video sealing key, as [`set_video_keys`](Self::set_video_keys) expanded
    /// it. A caller seals a whole frame's worth of records from one of these, so it is handed out
    /// by reference rather than rebuilt per record.
    fn video_seal_key(&self, connector: usize) -> Result<Arc<crate::cp::VideoKey>> {
        self.video_keys
            .lock()
            .get(connector)
            .ok_or(EINVAL)?
    ...
    fn build_ella_config_buf(&self, connector: usize) -> Result<KVec<u8>> {
        let vkey = self.video_seal_key(connector)?;
        ...
        crate::cp::seal_video_arm(&vkey, stream, 0x0000, seq, &config)
    }
    ...
    fn build_stream_open_buf(&self, connector: usize) -> Result<Option<KVec<u8>>> {
        ...
        Ok(Some(crate::cp::seal_video_arm(
            &vkey, stream, 0x0002, seq, &content,
        )?))
    }
    ...
    fn build_navarro_prologue_buf(&self, connector: usize) -> Result<KVec<u8>> {
        ...
        let sealed = crate::cp::seal_video_arm(&vkey, stream, 0x0000, seal_seq, &descriptor)?;
        ...
        buf.extend_from_slice(
            &crate::cp::seal_video_arm(&vkey, stream, 0x000e, seal_seq, &config)?,
            GFP_KERNEL,
        )?;
        Ok(buf)
    }
            ...
            crate::cp::VIDEO_ARM_BURST.iter().enumerate()
        {
            ...
            match i {
                2 | 3 => {
                    ...
                    let frame = crate::cp::seal_video_arm(&vkey, sub, aux, seal_seq, &content)?;
                    seal_seq += 1;
