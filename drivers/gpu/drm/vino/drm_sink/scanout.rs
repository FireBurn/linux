...
impl PixelSource {
    ...
    fn source_px(&self, sx: usize, sy: usize) -> (u16, u16, u16) {
        ...
        match &self.color {
            ...
            // precision and scaled back. That loses up to two low bits of a corrected pixel, but
            // ignoring the correction instead would leave a compositor's night-colour shift
            // silently unapplied on exactly the outputs most likely to be colour-managed. Revisit
            // with a 10-bit LUT if banding is ever measured on a corrected HDR connector.
...
/// that property is what let the fan-out reach ~7.4x.
///
/// Allocated once on first use and never destroyed. A `SetOnce` cannot be drained, so the queue
/// and its worker threads outlive a module unload: one `workqueue_struct` is leaked per load.
/// That is a deliberate trade for now and not a property to rely on -- `encode_across_cpus` joins
/// every chunk before it returns, so the queue does not in fact need to outlive the module, and
/// moving it into the module's own state would end the leak. What stops that being a one-line
/// change is `max_active` below: a queue per device would give two docks two queues of that width
/// each, which is a different machine-wide fan-out from the one that was measured.
///
...
fn encode_and_send_haar(
    ...
    let identity = rotation.angle() == plane::Rotation::ROTATE_0
        ...
        && !rotation.contains(plane::Rotation::REFLECT_Y);
