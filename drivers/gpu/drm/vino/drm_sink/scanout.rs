...
fn encode_and_send_wht(
    ...
    let (frames, next_seq) = match parallel {
        ...
        None if full => crate::video::wht::colour_frame_ep08(geom, w_pad, h_pad, seq0, head, px)?,
    ...
    for repeat in 0..repeat_count {
        ...
        if due {
            ...
            {
                vino_debug!(
                    "vino: scanout head={} CP status poll failed ({e:?})\n",
                    head
                );
            }
        }
            ...
            if bodies.len() == hashes.len() {
                if let Some((coords, strips)) = encoded {
                    ...
                    for (&(sx, sy), body) in coords.iter().zip(strips) {
                        let idx =
                            (sy >> geom.strip_h_shift()) * tiles_x + (sx >> geom.strip_w_shift());
