...
impl<T: DeviceAttributes> AttributeGroup<T> {
    ...
    pub fn register_root(
        ...
        Self::register_root_inner(name, module, ctx, |_| false)
    }
    ...
    fn register_root_inner(
        ...
        module: &'static ThisModule,
        ctx: impl PinInit<T, Error>,
        inject_failure: impl Fn(usize) -> bool,
    ) -> Result<KBox<Self>> {
        ...
        // Allocate and pin the complete owner before publishing any callback. If anything below
        // fails, its Drop implementation removes the successfully created files before `_ctx` is
        // released.
        ...
        for index in 0..group.attrs.len() {
            if inject_failure(index) {
                return Err(ENOMEM);
            }
            // SAFETY: `root` is registered and the attribute's stable backing storage is owned by
            // `group`. On failure, `group` drops and removes every earlier file first.
            to_result(unsafe { bindings::device_create_file(root, &group.attrs[index]) })?;
            group.created += 1;
        }
...
impl<T: DeviceAttributes> Drop for AttributeGroup<T> {
    fn drop(&mut self) {
        ...
        for da in &self.attrs[..self.created] {
            // SAFETY: each attribute in this prefix was successfully published on `self.root`.
            // Removing it prevents new callbacks and synchronizes with callbacks already running.
...
mod tests {
    ...
    #[test]
    fn partial_registration_drops_context_after_unwind() {
