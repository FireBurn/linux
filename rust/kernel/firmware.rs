...
pub mod upload {
    ...
    pub trait Upload: Sized {
        ...
        /// offered and be called again with the remainder. A count larger than `chunk.len()` is
        /// clamped to it: the core subtracts what is reported from its own remaining length, and
        /// an over-report would underflow that and push the next chunk past the end of the image.
        fn write(
            data: <Self::Data as ForeignOwnable>::Borrowed<'_>,
            image: &[u8],
            offset: u32,
        ...
        /// Undo whatever `prepare` set up.
        ///
        /// Runs once the transfer has finished, whether it succeeded, failed or was cancelled. A
        /// `prepare` that returns an error is expected to have unwound itself, because the core
        /// abandons the transfer there and this is not called.
