...
/// - `Self` has no drop glue, kernel pointers, or uninitialized padding which could be copied to
///   userspace; and
/// - every bit pattern supplied to [`EventChannel::send`] is valid for a bytewise userspace copy.
...
///
/// The anchor is reserved but never delivered. During file teardown, DRM core clears
/// `pending.file_priv` under `event_lock` before freeing the file. The channel checks the same field
/// under the same lock before every delivery.
#[repr(C)]
struct EventAnchor {
    pending: bindings::drm_pending_event,
