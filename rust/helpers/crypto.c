...
/*
 * AES-CTR over the in-tree library, on a key already prepared by
 * aes_prepareenckey(). aes_ctr() takes the same transparent union as
 * aes_encrypt(), so it is wrapped here. @ctr is the initial counter block and
 * is advanced past the data, as the C function leaves it. Encryption and
 * decryption are the same operation.
 */
__rust_helper void
rust_helper_aes_enckey_ctr(const struct aes_enckey *key, u8 *ctr, u8 *data,
			   size_t len)
{
	aes_ctr(data, data, len, ctr, key);
}
