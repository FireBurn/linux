...
On x86-64 the encoder can use an AVX2 block transform instead of the scalar
one. It is byte-exact against the scalar path, which remains the default and
the reference, and is enabled with ``simd_transform=1``.
...
* 2560x1440 CVT-RB at 60, 120 or 165 Hz; and
* 3840x2160 CVT-RB at 60 Hz.

Each profile also names the ceilings of its hardware: the highest pixel clock a
single head may carry, an optional refresh-rate cap where the vendor driver is
known to clamp, and a dock-wide pixel budget checked when more than one CRTC is
enabled. Unsupported timings are rejected during atomic validation instead of
being approximated on the wire.
